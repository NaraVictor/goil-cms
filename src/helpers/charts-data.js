...
export const salesChartData = ( sales ) => {
	const sortedList = arraySort( sales, "createdAt" );
	let finalList = [];
	...
	const chartLabels = finalList.map( ( s ) =>
		format( new Date( s.createdAt ), "EEE, MMM d" )
	);
	const values = finalList.map( ( s ) => s.amount_paid );
	return {
		labels: chartLabels,
		datasets: [
			{
...
export const expensesChartData = ( expenses ) => {
	...
	const values = finalList.map( ( e ) => e.amount );
...
export const incomeExpenseChartData = ( dataSource ) => {
	...
	const labels = {
		income: income?.map( ( i ) => i.createdAt ),
